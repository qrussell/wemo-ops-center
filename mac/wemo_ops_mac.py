...
# --- PATH SETUP (macOS Standard) ---
# We force the path to the User's Library folder
            ...
            if data["status"] == "OK":
                ...
                def to_local(utc_str):
                    try:
                        ...
                        # Mac handles astimezone() robustly
...
class WemoOpsApp(ctk.CTk):
    def __init__(self):
        ...
        # NOTE: On Mac, we generally rely on the LaunchAgent service for schedules,
        # but running this thread here allows "Stand Alone" usage without installing the service.
    ...
    def build_device_card(self, dev):
        ...
        meta_str = f"IP: {dev.host} | MAC: {mac} | FW: {fw}"
    ...
    def _rename_task(self, dev, new_name):
        try:
            dev.change_friendly_name(new_name)
            self.after(0, lambda: messagebox.showinfo("Success", "Renamed."))
            self.after(0, self.refresh_network)
        except: 
            try:
                dev.basicevent.ChangeFriendlyName(FriendlyName=new_name)
                self.after(0, self.refresh_network)
            except: pass
        ...
        for mode in enc_modes:
            for length in len_opts:
                try:
                    ...
                    self.log_prov("  > Fallback to Legacy Setup...")
                    try:
    ...
    def add_job(self):
        ...
        if sType == "Time (Fixed)":
            ...
            except ValueError: messagebox.showerror("Format Error", "Time must be HH:MM"); return
        else:
            if not val: val = "0"
            if not val.isdigit(): messagebox.showerror("Format Error", "Offset must be integer"); return
        ...
        try:
            ...
            if os.path.exists(airport_path):
                ...
                for line in output.split('\n'):
                    ...
                    if len(parts) > 0:
                        ssid_candidate = line.split()[0]
                        if "wemo" in ssid_candidate.lower() or "belkin" in ssid_candidate.lower():
                            wemos.append(ssid_candidate)
            else:
